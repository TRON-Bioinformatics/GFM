...
def parse_args():
    ...
    parser.add_argument('--split-df-path', type=str, help='Path to train/test split DataFrame pickle file')
    parser.add_argument('--split-dict-path', type=str, help='Path to train/test split dictionary pickle file (alternative to split_df_path)')
    parser.add_argument('--output-dir', type=str, help='Working directory for outputs')
    parser.add_argument('--graph-dir', type=str, help='Directory containing graph files (GO, PPI, etc.)')
    ...
    parser.add_argument('--vae-save-path', type=str, help='Full path to save VAE model (overrides output_dir + vae_name)')
    ...
    parser.add_argument('--pert-adata-path', type=str, help='Path to AnnData h5ad file for perturbation data (required for certain graph types)')
    ...
    parser.add_argument('--skip-model-training', action='store_true', help='Skip GFM model training')
    parser.add_argument('--skip-prediction', action='store_true', help='Skip generating predictions')
    ...
    parser.add_argument('--use-condition-classifier', action='store_true', help='Use condition classifier loss')
    parser.add_argument('--use-null-embedding', action='store_true', help='Use null embedding in model')
    parser.add_argument('--use-scvi-vae', action='store_true', help='Use scvi-tools VAE instead of custom VAE implementation')
    parser.add_argument('--save-times', action='store_true', help='Save training and inference times to timing.json in working directory')
    parser.add_argument('--aggregation-method', type=str, choices=['sum', 'deepset'], help='Aggregation method for GNN (default: sum)')
...
def main():
    ...
    if not args.adata_path or (not args.split_df_path and not args.split_dict_path) or not args.output_dir:
        raise ValueError("Required: --adata-path, --split-df-path, --working-dir (or provide --config)")
    ...
    gfm.initialize_fm(randomize_graph=args.randomize_graph, 
