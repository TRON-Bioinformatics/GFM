...
def train_vae(
    ...
    if return_training_history:
        training_history = {"train_loss": [], "recon_loss": [], "kld": [], "beta": []}
        if use_validation:
    ...
    for epoch in range(max_epochs):
        ...
        train_loss, recon_loss, kld = train_one_epoch_vae(
            vae, train_loader, optimizer, device=device, batch_size=batch_size, beta=beta
        )

        # Optional validation
        if use_validation:
            val_loss, val_recon_loss, val_kld = evaluate_vae(
...
def make_condition_aware_vae_dataloader(
    adata, batch_size=128, shuffle=True, device="cpu", indices=None, covariate_columns=["condition"]
):
    """
    ...
    condition_indices = torch.tensor(
        [group_to_idx[c] for c in adata.obs.loc[indices, "group"].values], dtype=torch.long
    )
...
def train_one_epoch_scvi_vae(vae, train_loader, optimizer, device="cpu", batch_size=128, beta=1.0):
    vae.train()
    total = 0.0
