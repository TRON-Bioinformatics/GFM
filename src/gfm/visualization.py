...
def plot_correlation_dotplot(
    ...
    x_cols_labels=[
        "Number of training-set neighbors",
        "Fraction of training-set neighbors",
        "Maximum Similarity",
        "Median Similarity",
    ],
    save_path=None,
    ...
    for rho_val in rho_values:
        legend_elements.append(
            plt.scatter([], [], s=rho_val * 300, c="gray", alpha=0.7, label=f"{rho_val:.1f}")
        )
...
def get_adata_condition_subset(adata_path_dict, condition):
    ...
    for model, path in adata_path_dict.items():
        ...
        if model == "Truth":
            ...
            pert = "+".join(
                [pert_gene for pert_gene in condition.split("+") if pert_gene != "ctrl"]
            )
            filt = adata.obs["condition"] == pert
            adata_subset = adata[filt].to_memory()
            if adata_subset.n_obs == 0:
                print(
                    f"Warning: No cells found for model '{model}' under condition '{condition}' (looking for perturbation '{pert}'). Skipping this model."
                )
                missing_models.append(model)
                continue
            adata_subset.obs["model"] = model
...
def plot_delta_expression_boxplot_model(
    ...
    for model_name in model_name_list:
        ...
        if model_name == "Truth":
            df = adata[
                (adata.obs["condition"] == cond) & (adata.obs["model"] == model_name), top_genes
            ].to_df()
    ...
    combined_plot_df["Model"] = pd.Categorical(
        combined_plot_df["Model"], categories=model_name_list
    )
    ...
    if isinstance(palette, list):
        legend_elements = [
            Patch(facecolor=palette[i], label=model_name)
            for i, model_name in enumerate(model_name_list)
        ]
    else:
        legend_elements = [
            Patch(facecolor=palette[model_name], label=model_name) for model_name in model_name_list
        ]
    fig.legend(
        handles=legend_elements,
        loc="lower center",
