...
from gfm.helpers import (
    SplitHandler,
    build_graph,
    get_cell_embedding,
    make_condition_labels_graph,
    make_condot_data_loader,
    make_data_loader,
    make_prediction_data_loader,
)
...
from gfm.vae_training_utils import (
    ...
    make_scvi_vae_dataloader,
    make_vae_dataloader,
    train_condition_aware_vae,
    train_scvi_vae,
    train_vae,
)
...
class GFM:
    ...
    def initialize_fm(
        ...
        if graph_type == "one_hot":
            self.pert_names_graph = np.array(
                [
                    cond.replace("+", "").replace("ctrl", "")
                    for cond in self.pert_names
                    if cond != "ctrl"
                ]
            )
            self.condition_labels_graph = make_condition_labels_graph(
                self.pert_names, self.pert_names_graph
            )

            self.model = ConditionalODE(
                pert_input_size=len(self.pert_names_graph), pert_encoding=graph_type, **common_args
            ).to(self.device)
        ...
        if not self.use_scvi_vae:
            try:
                ...
                if not self.use_contrastive and not self.use_condition_classifier:
                    ...
                    train_loader = make_vae_dataloader(
                        self.adata,
                        batch_size=batch_size,
                        shuffle=True,
                        device=self.device,
                ...
                # Condition-aware VAE (with contrastive and/or classifier)
                    ...
                    print(
                        f"Training condition-aware VAE (contrastive={self.use_contrastive}, classifier={self.use_condition_classifier})"
                    )
                ...
                if self.use_contrastive or self.use_condition_classifier:
                    ...
                    self.adata.obsm["X_vae"] = get_cell_embedding(
                        self.condition_aware_vae, self.adata, device=self.device
                    )
                    print("VAE embeddings stored in adata.obsm['X_vae']")

                else:
                    torch.save(self.vae.state_dict(), self.vae_save_path)
                    print(f"VAE model saved to {self.vae_save_path}")
                    self.adata.obsm["X_vae"] = get_cell_embedding(
                        self.vae, self.adata, device=self.device
                    )
                    print("VAE embeddings stored in adata.obsm['X_vae']")
            ...
            datasplitter_kwargs = {"external_indexing": [train_idx, val_idx, rest_idx]}
            ...
            try:
                ...
                print("SCVI VAE embeddings stored in adata.obsm['X_vae']")
    ...
    def prepare_training(self, batch_size=1000, lr=1e-3, ot_replace=True, use_condot=False):
        ...
        if self.use_my_scvi_vae:
            if self.vae_save_path is not None and os.path.exists(self.vae_save_path):
                ...
                self.my_scvi_vae.load_state_dict(
                    torch.load(self.vae_save_path, map_location=self.device)
                )
            ...
            if self.vae_save_path is not None:
                if os.path.exists(self.vae_save_path):
                    ...
                    if self.use_contrastive or self.use_condition_classifier:
                        self.condition_aware_vae.load_state_dict(
                            torch.load(self.vae_save_path, map_location=self.device)
                        )
                    else:
                    ...
                    if "X_vae" not in self.adata.obsm:
                        ...
                        print("VAE model loaded and embeddings stored in adata.obsm['X_vae']")
        else:
            self.scvi_vae = self.scvi_vae.load(
                self.output_dir, prefix=self.vae_name, adata=self.adata
            )
    ...
    def train_model(self, max_epochs=500, early_stopping_patience=5, eval_freq=10, save_path=None):
                ...
                for epoch in range(max_epochs):
                    if self.no_fm:
                        ...
                        if self.use_condot:
                            train_loss = train_one_epoch_condot(
                                self.model, self.train_loader, self.optimizer, self.path
                            )
                            val_loss = evaluate_one_epoch_condot(
                                self.model, self.val_loader, self.path
                            )
                        else:
                    ...
                    if epoch % eval_freq == 0 or epoch == max_epochs - 1:
                        if self.no_fm:
                            ...
                            if self.use_condot:
                                val_mse, val_w2d, val_mmd = evaluate_metrics_condot(
                                    self.model, self.val_loader
                                )
                            else:
                                val_mse, val_w2d, val_mmd = evaluate_metrics(
                                    self.model, self.val_loader
                                )
                        ...
                        print(
                            f"Epoch {epoch + 1}/{max_epochs}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Val Error: {val_mse:.4f}, Val W2D: {val_w2d:.4f}, Val MMD: {val_mmd:.4f}"
                        )
    ...
    def predict(self, pred_df, n_cells=100, ctrl_name="ctrl", guidance=1.5):
        """
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                if len(self.split.covariate_columns) == 2:
                    if needs_library:
                        z0, y, c, library = batch
                        c = c.to(device=device, non_blocking=True)
                        library = library.to(device=device, non_blocking=True)
                    else:
                        z0, y, c = batch
                        c = c.to(device=device, non_blocking=True)
                else:
                ...
                if self.no_fm:
                    ...
                    z1 = solver.sample(time_grid=T, x_init=z0, step_size=None, method="dopri5")

                if self.use_scvi_vae:
                    generative_inputs = self._prepare_scvi_generative_inputs(z1, library)
                    ...
                    # `library` is log-library-size sampled from ctrl cells (workaround:
                    # flow matching does not predict library latent).
                    x_counts = self.my_scvi_vae.sample_expression(z1, library)
    ...
    def _prepare_scvi_generative_inputs(self, z1, library):
        ...
        generative_inputs = {
            "z": z1,
            "library": library,
            "y": torch.zeros_like(library, dtype=torch.long).to(
                z1.device
            ),  # Dummy condition labels (not used in generative mode)
