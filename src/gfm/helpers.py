...
def _download_file_if_missing(url, save_path):
    ...
    raise RuntimeError(
        f"Failed to download artifact to {save_path}. Last error: {last_error}"
    )
...
class SplitHandler:
    def __init__(self, split_dict_path=None, split_dict=None, split_df_path=None, split_df=None):
        ...
        if provided_params == 0:
            raise ValueError("Please provide exactly one of: split_dict_path, split_dict, split_df_path, or split_df.")
        elif provided_params > 1:
            raise ValueError("Please provide only one of: split_dict_path, split_dict, split_df_path, or split_df.")
        ...
        if split_df_path is not None:
            if os.path.exists(split_df_path):
                ...
                if split_df_path.endswith('.csv'):
                    ...
                    raise ValueError(f"Unsupported file format. Use .csv, .tsv, .pkl, or .pickle")
            else:
    ...
    def add_split_to_adata(self, adata):
        ...
        if missing_cols:
            raise ValueError(f"Covariate columns {missing_cols} not found in adata.obs. "
        ...
        merged = obs_df.merge(
            self.split_df[self.covariate_columns + ['split']], 
            on=self.covariate_columns, 
            how='left'
        )
        
        # Check if merge was successful
        if 'split' not in merged.columns:
            raise ValueError(f"Merge failed - 'split' column not found in result. "
...
def sample_x0_from_ctrl(adata, num_cells, ctrl_name='ctrl', additional_filt=None,
                        selected_genes=None):
    """
...
def sample_x0_from_ctrl_tensor(adata, num_cells, ctrl_name='ctrl', additional_filt=None,
                               device='cpu'):
...
def sample_x0_from_train_non_ctrl(adata, num_cells, ctrl_name='ctrl', additional_filt=None,
                                   selected_genes=None):
    """
...
def sample_z0_from_ctrl(adata, num_cells, ctrl_name='ctrl', additional_filt=None,
                        selected_genes=None):
...
def sample_z0_from_ctrl_tensor(adata, num_cells, ctrl_name='ctrl', additional_filt=None,
                               device='cpu'):
...
def make_data_loader(adata, ot_sampler, condition_labels, context_to_idx=None, split='train', covariate_columns=['condition'],
                     batch_size=32, shuffle=True, device='cpu', train_with_ctrl=False,
                     flow_from_ctrl=True, ot_replace=False):

    ot_chunk_size = 10000

    with torch.no_grad():
        print(f"Preparing data loader for split='{split}' with flow_from_ctrl={flow_from_ctrl} and train_with_ctrl={train_with_ctrl}...")
        split_filt = adata.obs['split'] == split
        if flow_from_ctrl:
            ctrl_filt = adata.obs['condition'] == 'ctrl'
        z0_coupled_list = []
        z1_coupled_list = []
        y_list = []
        ...
        for group in unique_groups:
            ...
            if flow_from_ctrl:
                if not train_with_ctrl and condition == 'ctrl':
                    ...
                    # Sample on CPU
                    z0_np = sample_z0_from_ctrl(adata, z1.shape[0], ctrl_name='ctrl', additional_filt=additional_filt)
...
def make_condot_data_loader(adata, condition_labels, context_to_idx=None, covariate_columns=['condition'],
                     split='train',batch_size=32, shuffle=True, device='cpu'):
    ...
    with torch.no_grad():
        ...
        for group in unique_groups:
            ...
            if flow_from_ctrl:
                ...
                # Sample on CPU
                z0_np = sample_z0_from_ctrl(adata, n_cells, ctrl_name=ctrl_name, 
            ...
            if include_library:
                ...
                library_np = sample_x0_from_ctrl(adata, n_cells, ctrl_name=ctrl_name, 
                                                additional_filt=additional_filt)
                library = torch.tensor(library_np, dtype=torch.float32).sum(dim=1).log().unsqueeze(1)
...
def get_go_graph(device, k=None, graph_dir="./data", randomize=False):
    ...
    edge_list = pd.concat([group.nlargest(k + 1, 'importance') for _, group in df_jaccard.groupby('target')]).reset_index(drop=True)
...
def get_ppi_graph(k=None, device='cpu', graph_dir="./data"):
    ...
    if not os.path.exists(csv_path):
        ...
        results = mg.querymany(unique_proteins, scopes='ensembl.protein', fields='symbol', species='human', verbose=False)
        ...
        ppi_net = ppi_net.rename(columns={'gene1': 'source', 'gene2': 'target', 'combined_score': 'importance'})
    ...
    edge_list = pd.concat([group.nlargest(k + 1, 'importance') for _, group in ppi_net.groupby('target')]).reset_index(drop=True)
    ...
    valid_edges = edge_list[
        edge_list['source'].isin(gene_to_idx) & 
        edge_list['target'].isin(gene_to_idx)
    ]
    ...
    edge_index = torch.tensor(np.array([source_indices, target_indices]), dtype=torch.long).to(device)

    return edge_index, edge_weight, edge_list,  pert_names_graph

def get_pert_similarity_graph(pert_adata_path=None, graph_dir="./data", pert_column=None, ctrl_name=None,k=None, device=None):
    k = 20 if k is None else k
    ...
    if os.path.exists(os.path.join(graph_dir, f"pert_graph_edge_list_{k}.csv")) and \
        os.path.exists(os.path.join(graph_dir, f"pert_graph_edge_index_{k}.pt")) and \
        os.path.exists(os.path.join(graph_dir, f"pert_graph_edge_weight_{k}.pt")) and \
        os.path.exists(os.path.join(graph_dir, f"pert_graph_pert_names_graph_{k}.npy")):
        ...
        edge_weight = torch.load(os.path.join(graph_dir, f"pert_graph_edge_weight_{k}.pt")).to(device)
        ...
        if pert_adata_path is None:
            raise ValueError("pert_adata_path must be provided when building perturbation similarity graph.")
        ...
        if ctrl_name not in mean_expr_per_condition:
            raise ValueError(f"Control condition '{ctrl_name}' not found in adata.obs['{pert_column}']")
        ...
        for pert in pert_names_graph:
            ...
            mean_pert = np.mean([mean_expr_per_condition[cond] for cond in matching_conditions], axis=0)
...
def build_graph(
    ...
    if graph_dir is None:
        graph_dir = "./data"
    if graph_type == 'go':
        edge_index, edge_weight, edge_list, pert_names_graph = get_go_graph(device, k=k, graph_dir=graph_dir, randomize=randomize_graph)
        condition_labels_graph = make_condition_labels_graph(pert_names, pert_names_graph)
    elif graph_type == 'ppi':
        edge_index, edge_weight, edge_list, pert_names_graph = get_ppi_graph(k=k, device=device, graph_dir=graph_dir)
        condition_labels_graph = make_condition_labels_graph(pert_names, pert_names_graph)
        ...
        if pert_encoding == 'hgnn':
            ...
            edge_index_go, edge_weight_go = get_edge_index_and_weight(edge_list, pert_names_graph, relation_type='go_sim', device=device)
            edge_index_ppi, edge_weight_ppi = get_edge_index_and_weight(edge_list, pert_names_graph, relation_type='ppi', device=device)
            edge_index = {
            ...
            edge_index, edge_weight = get_edge_index_and_weight(edge_list, pert_names_graph, device=device)

        condition_labels_graph = make_condition_labels_graph(pert_names, pert_names_graph)
    elif graph_type == 'pert':        
        edge_list, edge_index, edge_weight, pert_names_graph = get_pert_similarity_graph(
            pert_adata_path=pert_adata_path, graph_dir=graph_dir,  pert_column='perturbation', ctrl_name='control', device=device, k=20)
        condition_labels_graph = make_condition_labels_graph(pert_names, pert_names_graph)
        ...
        if pert_encoding == 'hgnn':
            ...
            edge_index_go, edge_weight_go = get_edge_index_and_weight(edge_list, pert_names_graph, relation_type='go_sim',device=device)
            edge_index_pert, edge_weight_pert = get_edge_index_and_weight(edge_list, pert_names_graph, relation_type='pert_sim', device=device)
            edge_index = {
                ('gene', 'go_sim', 'gene'): edge_index_go,
...
def get_pert_graph_info(gfm, test_conds):
    ...
    perts_in_train = set([item for cond in gfm.split.split_dict['train'] for item in trim_cond(cond).split('+')])
...
def calculate_delta_result(result_df, result_no_pred, result_pos_ctrl=None, normalize=False, return_detail=False):
    """
    ...
    test_result = result_df.set_index(index_cols).reindex(
        result_no_pred.set_index(index_cols).index
    ).reset_index()
    result_no_pred = result_no_pred.set_index(index_cols).reindex(
        result_no_pred.set_index(index_cols).index
    ).reset_index()
    ...
    if normalize:
        ...
        dynamic_range = result_pos_ctrl.iloc[:, num_index_cols:] - result_no_pred.iloc[:, num_index_cols:]
...
def _compute_metrics_single_group(
    ...
    filt_pred = (adata_pred.obs[condition_col] == pert) & (adata_pred.obs[context_col] == cond_values[context_col]) if has_context else (adata_pred.obs[condition_col] == pert)
    if layer is None:
    ...
    if control_type == 'ctrl':
        ctrl = sample_x0_from_ctrl(adata, n_pred, ctrl_name='ctrl', additional_filt=additional_filt, 
                               selected_genes=top_genes)
    elif control_type == 'pert_train':
        ctrl = sample_x0_from_train_non_ctrl(adata, n_pred, ctrl_name='ctrl', additional_filt=additional_filt, 
                                             selected_genes=top_genes)
    ...
    with warnings.catch_warnings():
        ...
        result_row['pearson_delta'] = pearsonr(truth.mean(0) - ctrl.mean(0), pred.mean(0) - ctrl.mean(0))[0]
    ...
    if n_jobs == 1:
        ...
        for group in tqdm(unique_groups, desc="Computing metrics"):
            result_row = _compute_metrics_single_group(
                group, adata, adata_pred, control_type, covariate_columns,
                condition_col, context_col, condition_col_pos, context_col_pos,
                has_context, adata_pred_genes, top_n, layer, is_drug, n_pred
            )
        ...
        if is_backed:
            print(f"Detected backed AnnData - using threading backend for parallelization (n_jobs={n_jobs})...")
            backend = 'threading'
        else:
        ...
        results_list = Parallel(n_jobs=n_jobs, backend=backend, verbose=10)(
            delayed(_compute_metrics_single_group)(
                group, adata, adata_pred, control_type, covariate_columns,
                condition_col, context_col, condition_col_pos, context_col_pos,
                has_context, adata_pred_genes, layer, top_n, is_drug, n_pred
            )
            for group in unique_groups
        )
...
def _compute_no_change_single_group(
    ...
    if control_type == 'ctrl':
        pred = sample_x0_from_ctrl(adata, n_pred, ctrl_name='ctrl', additional_filt=additional_filt, 
                                selected_genes=top_genes)
        ctrl = pred
    elif control_type == 'pert_train':
        pred = sample_x0_from_train_non_ctrl(adata, n_pred, ctrl_name='ctrl', additional_filt=additional_filt, 
                                             selected_genes=top_genes)
        ctrl = pred
    else:
    ...
    if n_jobs == 1:
        ...
        for group in tqdm(unique_groups, desc="Computing no-change metrics"):
            result_row = _compute_no_change_single_group(
                group, adata, control_type, covariate_columns,
                condition_col, context_col, condition_col_pos, context_col_pos,
                has_context, top_n, n_pred
            )
            if result_row is not None:
                results_list.append(result_row)
    else:
...
def compute_metrics_over_de(adata, adata_pred, pred_df, control_type='ctrl', covariate_columns=['condition'], 
                           compute_baseline=True, save_dir=None, suffix='', n_jobs=1):
    """
    ...
    if compute_baseline and save_dir is not None:
        baseline_results.to_csv(os.path.join(save_dir, f"results_no_change_over_n{suffix}.csv"), index=False)
...
def _perform_paired_sample_test(
    ...
    non_metric_cols = [
        ...
        if col not in set(metrics) | {'model'}
    ]
    ...
    for model in comparison_models:
        for metric in metrics:
            ref_data = result_df[result_df['model'] == reference_model][pairing_cols + [metric]].copy()
            ...
            if len(paired_data) < 2:
                print(f"Warning: Not enough paired samples for {model} vs {reference_model} on {metric}")
                continue

            alt = 'two-sided' if alternative == 'two-sided' else better_alternative.get(metric, 'two-sided')
            if method == 'wilcoxon':
...
def _aggregate_results_by_split(result_df, metrics=None, groupby_cols=None):
    ...
    aggregated = result_df.groupby(groupby_cols, observed=True)[metrics].agg(['mean', 'sem']).reset_index()
    aggregated.columns = [
...
def combine_results(
    ...
    if calculate_delta:
        if baseline_model not in categories:
            raise ValueError(f"Invalid baseline_model: {baseline_model}. "
            ...
            for name in names:
                ...
                if isinstance(result_dict[name], list):
                    ...
                    for i, res_path in enumerate(result_dict[name]):
                        ...
                        delta_out = calculate_delta_result(result, result_no_pred, result_pos_ctrl=result_pos_ctrl, normalize=normalize)
            ...
            for name in names:
                ...
                if isinstance(result_dict[name], list):
                    ...
                    for i, res_path in enumerate(result_dict[name]):
                        result = read_metric_csv(res_path)
                        result_no_pred = read_metric_csv(result_dict[baseline_model][i])
                        delta_out = calculate_delta_result(result, result_no_pred, normalize=normalize)
                        result_delta = delta_out[0] if isinstance(delta_out, tuple) else delta_out
                        result_delta['split'] = i + 1
                        result_delta_list.append(result_delta)
                    if result_delta_list:  # Only concatenate if list is not empty
...
def multi_hot_label(label_sets, num_classes, normalization=False, device: str | torch.device = 'cpu'):
    """
    ...
    with torch.no_grad():
        ...
        for cond, ct in unique_pairs:
            if cond not in condition_labels.keys():
                ...
                mohe = multi_hot_label(condition_labels[cond], len(pdata_train.var_names), device=device)
                pert_emb_cond = torch.matmul(mohe, pert_emb_tensor)

                x_truth = pdata_train[(pdata_train.obs['condition'] == cond) & (pdata_train.obs['cell_type'] == ct)].X
                x_ctrl = pdata_train[(pdata_train.obs['condition'] == 'ctrl') & (pdata_train.obs['cell_type'] == ct)].X
...
def train_and_predict_linear_model(adata, drug_graph=False, device='cpu'):
    ...
    condition_labels = make_condition_labels_graph(pert_names, pdata_train.var_names, drug_graph=drug_graph)
    ...
    val_loader = make_LM_data_loader(
        pdata_val, 
        condition_labels,
        pert_emb_tensor,
        device=device)
    ...
    # Initialize model
    model = LinearPerturbationModel(n_pc=pert_emb_tensor.shape[1], gene_dim=pdata_train.shape[1]).to(device)
    ...
    with torch.no_grad():
        ...
        for cond, ct in unique_pairs:
            if cond not in condition_labels.keys():
                ...
                mohe = multi_hot_label(condition_labels[cond], len(pdata_test.var_names), device=device)
                pert_emb_cond = torch.matmul(mohe, pert_emb_tensor)
                ...
                n_cells = sum((pdata_test.obs['condition'] == cond) & (pdata_test.obs['cell_type'] == ct))
                cond_name = '+'.join([p for p in cond.split('+') if p != 'ctrl'])
                cond_list.extend([cond_name] * n_cells)
                ct_list.extend([ct] * n_cells)
        
        adata_pred_lm = sc.AnnData(np.vstack(pred_list), obs={'condition': cond_list, 'cell_type': ct_list})
...
def run_additive_model(adata, split_dict_subgroup_path, result_path):
    ...
    single_pert_set = set([item for cond in split_dict_subgroup['test_subgroup']['combo_seen2'] for item in cond.split('+')])
    ...
    pert_to_single_cond = {
        pert: next((cond for cond in adata.obs['condition'].unique()
                    if pert in cond and "ctrl" in cond), None)
    ...
    for pert in split_dict_subgroup['test_subgroup']['combo_seen2']:
        ...
        if single_pert_1 is None and single_pert_2 is not None:
            dX_p2 = adata[adata.obs['condition'] == single_pert_2, :].X.toarray().mean(axis=0) - ctrl
            X_pred = ctrl + dX_p2
        elif single_pert_2 is None and single_pert_1 is not None:
            dX_p1 = adata[adata.obs['condition'] == single_pert_1, :].X.toarray().mean(axis=0) - ctrl
            X_pred = ctrl + dX_p1
        elif single_pert_1 is None or single_pert_2 is None:
            print(f"Could not find single perturbation condition for {pert}, use control as prediction")
            X_pred = ctrl
        else:
            dX_p1 = adata[adata.obs['condition'] == single_pert_1, :].X.toarray().mean(axis=0) - ctrl
            dX_p2 = adata[adata.obs['condition'] == single_pert_2, :].X.toarray().mean(axis=0) - ctrl
